		...
		if (result == null) {
			...
			CompositeImageDescriptor compositImageDesc = new CompositeImageDescriptor() {
				...
				protected void drawCompositeImage(int width, int height) {
					CachedImageDataProvider prov = createCachedImageDataProvider(baseImage);
					CachedImageDataProvider provDec = createCachedImageDataProvider(decorator);
					
					drawImage(prov, 0, 0);
					if (corner == TOP_LEFT) {
						drawImage(provDec, 0, 0);
					} else if (corner == TOP_RIGHT) {
						drawImage(provDec, bib.width - dib.width, 0);
					} else if (corner == BOTTOM_LEFT) {
						drawImage(provDec, 0, bib.height - dib.height);
					} else if (corner == BOTTOM_RIGHT) {
						drawImage(provDec, bib.width - dib.width, bib.height - dib.height);
					}
				}
				@Override
